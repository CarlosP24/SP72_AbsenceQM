function figure_conductance(temp::String = ""; Φs = [0.65, 0.88], color_loop1 = [:lightgreen, :lightblue], color_loop2 = [:yellow, :pink], χs = [5, 100])
    ...
    scatter!(ax, 55, -0.015; color = color_loop1[1], marker = :circle, markersize = 10)

    vlines!(ax, χs, ymax = 0.1, color = color_loop2, linewidth = 4 )

    ax = Axis(fig[1, 2]; xlabel, ylabel)
    mGQ, MGQ = plot_conductance(ax, "base_fs_szoom"*temp, "QMajo"; colorrange = (0, 2) )
    ...
    scatter!(ax, 55, -0.015; color = color_loop1[2], marker = :circle, markersize = 10)

    vlines!(ax, χs, ymax = 0.1, color = color_loop2, linewidth = 4 )


    Colorbar(fig[1, 3], colormap = :thermal, limits = (0, 2), ticks = [0, 2], label = L"$G$ (e^2/h)", labelpadding = -15)

    xlabel = L"$\Phi / \Phi_0$"
    ax = Axis(fig[2, 1]; xlabel, ylabel)
    mGM, MGM = plot_conductance_Phi(ax, "base_fs_szoom"*temp*"_Phis", 5; colorrange = (0, 2))
    ...
    mGQ, MGQ = plot_conductance_Phi(ax, "base_fs_szoom"*temp*"_Phis", 100; colorrange = (0, 2))
    Φ = plot_TT(ax, "base_fs"; linewidth = 2, color = :white)
    text!(ax, 1, -0.022; text = L"\Phi^\text{c}", color = :white, align = (:center, :center))
    ...
    scatter!(ax, 1.3, 0.015; color = color_loop2[2], marker = :circle, markersize = 10)
